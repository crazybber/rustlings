...
mod tests {
    ...
    fn test_iterate_string_vec() {
        ...
        let v1_iter = words.iter();

        // using a map opertion
    ...
    fn test_iterate_into_string() {
        let words = vec!["hello", " ", "world"];
        let capitalized_words = words.into_iter().filter(|x| x.is_empty()).flatmap(|x| capitalize_first(x)).collect();
        assert_eq!(capitalized_words, "Hello World");
    }
